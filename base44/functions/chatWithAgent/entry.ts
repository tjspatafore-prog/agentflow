...
Deno.serve(async (req) => {
  try {
    ...
    // All users can access all agents — WhatsApp users connect to the admin's agents directly
    // (the nexus_assistant gateway handles routing)
...
async function callOpenAI(apiKey, model, messages, tools, executeTool, baseUrl) {
  ...
  while (toolCallCount < maxToolCalls) {
    ...
    // gpt-5.6-sol is a reasoning model that does not support function tools in /v1/chat/completions
    if (tools && tools.length > 0 && model !== 'gpt-5.6-sol') {
      body.tools = tools;
